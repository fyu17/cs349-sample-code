    ...
    override fun start(stage: Stage?) {
        ...
        shape.setOnMouseDragged { event ->
            if (state == STATE.DRAG) {
                ...
                if (shape.boundsInParent.minX + dx >= 0.0 && shape.boundsInParent.maxX + dx <= SCREEN_WIDTH) {
                    shape.x += dx
                    startX = event.x
                }

                if (shape.boundsInParent.minY + dy >= 0.0 && shape.boundsInParent.maxY + dy <= SCREEN_HEIGHT) {
                    shape.y += dy
                    startY = event.y
                }
            }
